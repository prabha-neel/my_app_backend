...
from .models import Organization, SchoolAdmin, AdmissionRequest # <--- AdmissionRequest yahan add kiya
...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if profile:
            # Handle if school_admin_profile is a related manager
            if hasattr(profile, 'first'):
    ...
    # ✅ Action Moved Here: Logic same rakha hai
    ...
    def reject_admission(self, request, pk=None):
        ...
        # Check authorization
        admin_profile = request.user.school_admin_profile.filter(organization=admission_req.organization).exists()
...
class SchoolAdminViewSet(viewsets.ModelViewSet):
    ...
    def toggle_active_status(self, request, pk=None):
        ...
        return Response({'is_active': profile.is_active, 'message': 'Status updated.'})
...
class AdminDashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        if not school and hasattr(request.user, 'school_admin_profile'):
            profile = request.user.school_admin_profile.first()
        ...
        # 4. Teacher Attendance (Static for now)
        ...
        # ✅ 7. REAL NOTIFICATION LOGIC
        # Yahan humne 'Notification' model use kiya hai
        ...
        notices_data = [
            {
                "title": n.title,
                "subtitle": n.message[:50] + "..." if n.message else "", 
        ...
        return Response({
            ...
            "fee_stats": {
                "today": f"{today_fee:,.0f}",
                "monthly": f"{monthly_fee:,.0f}"
            },
