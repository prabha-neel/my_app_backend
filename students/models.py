...
class StudentProfile(models.Model):
    ...
    # StudentProfile class ke andar ye line add karo
    transport_route = models.ForeignKey(
        'transport.BusRoute', # Ensure karo ki 'transport' app aur 'TransportRoute' model exist karta ho
        on_delete=models.SET_NULL, 
        null=True, 
        blank=True,
        related_name='students'
...
class ParentConnection(models.Model):
    ...
    class Meta:
        unique_together = ('user', 'student') # Ek banda ek bache ko ek hi baar request bhej sake

    def __str__(self):
        return f"{self.user.username} -> {self.student.student_unique_id} ({self.status})"