...
# 1. Azad Parinda (Normal User) yahan se request bhejega
class SendParentRequestView(generics.CreateAPIView):
    serializer_class = ParentRequestSerializer
    permission_classes = [IsAuthenticated]

# 2. Bache ke liye: Apni pending requests ki list dekhna
class StudentPendingRequestsView(generics.ListAPIView):
    serializer_class = ParentRequestSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        # Sirf wahi requests jo is bache ke liye aayi hain aur PENDING hain
        return ParentConnection.objects.filter(
            student__user=self.request.user, 
            status='PENDING'
        )

# 3. Bache ke liye: Request Accept/Reject karna (Tera wahi code, thoda polished)
class StudentActionRequestView(generics.UpdateAPIView):
    permission_classes = [IsAuthenticated]
    queryset = ParentConnection.objects.all()
...
class ParentDashboardView(generics.RetrieveAPIView):
    """
    Objective: Jab Parent dashboard khole, toh use bache ka saara data dikhe.
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        connection = ParentConnection.objects.filter(user=user, status='ACCEPTED').select_related('student__organization').first()
        
        if not connection:
            return Response({"error": "Bhai, pehle kisi bache se connect toh ho jao!"}, status=404)

        student = connection.student
        
        # 2. Saara data ek hi jagah assemble karo
        # Hum pehle se bane serializers reuse karenge
        data = {
            "child_info": StudentProfileSerializer(student).data,
            "organization_name": student.organization.name,
            "academic_results": StudentResultSerializer(student.results.all(), many=True).data,
            "fee_details": StudentFeeSerializer(student.fees.all(), many=True).data,
        }

        # 3. Transport Details (Jiska tune pucha tha)
        # Agar StudentProfile mein transport linked hai:
        if hasattr(student, 'transport_route') and student.transport_route:
            route = student.transport_route
            data["transport"] = {
                "bus_no": route.bus_number,
                "driver": route.driver_name,
                "contact": route.driver_mobile,
